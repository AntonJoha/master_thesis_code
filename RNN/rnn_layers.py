...
class PredictTime(nn.Module):
    ...
    def make_modules(self):
        for i in range(self.hidden_layers):
            self.grus.append(nn.LSTMCell(input_size=self.hidden_size, hidden_size=self.hidden_size).to(self.device))
    ...
    def random_state(self):
        self.hidden = []
        for i in range(self.hidden_layers):
            self.hidden.append(torch.randn(self.hidden_size, device=self.device)) 
        self.hidden1 = torch.randn(1, self.hidden_size, device=self.device)
