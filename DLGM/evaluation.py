...
def get_yhat(m,m_r, x, forcing=True, seq_len=1):
    res = []
    m.eval()
    prev = x[0]
    for i in x:
        if forcing:
            rec = m_r(i.unsqueeze(0))
            m.set_xi(rec[-1])
        else:
            m.make_xi()
        val= m()
...
def evaluate_model(m,m_r,x,y, x_test,y_test,conf, draw_images=True):
    ...
    print("Initial")
    y_hat_f = get_yhat(m, m_r,x, forcing=True, seq_len=conf["seq_len"])    
    y_hat = get_yhat(m, m_r,x,  forcing=False, seq_len=conf["seq_len"])
    print("Initial_done")
    y_hat_sum = sum(y_hat)
    y_hat_f_sum = sum(y_hat_f)
    
    y_hat_test_f = get_yhat(m, m_r,x_test, forcing=True , seq_len=conf["seq_len"])
    y_hat_test = get_yhat(m,m_r, x_test, forcing=False , seq_len=conf["seq_len"])
    ...
    print("HERE")
    for i in range(10):
        y_hat_f_t = get_yhat(m,m_r,  x,  forcing=True , seq_len=conf["seq_len"])    
        y_hat_t = get_yhat(m,m_r, x, forcing=False, seq_len=conf["seq_len"])
        y_hat_sums.append(sum(y_hat_t).cpu().numpy().tolist())
        y_hat_f_sums.append(sum(y_hat_f_t).cpu().numpy().tolist())
        y_hat_test_f_t = get_yhat(m,m_r,x_test, forcing=True, seq_len=conf["seq_len"])
        y_hat_test_t = get_yhat(m,m_r, x_test,  forcing=False, seq_len=conf["seq_len"])
